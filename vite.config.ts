...
function sebufApiPlugin(): Plugin {
  ...
  async function buildRouter() {
    const [
      ...
      emergencyServerMod, emergencyHandlerMod,
    ] = await Promise.all([
        import('./server/router'),
        import('./server/cors'),
        import('./server/error-mapper'),
        import('./src/generated/server/worldmonitor/emergency/v1/service_server'),
        import('./server/worldmonitor/emergency/v1/handler'),
      ]);

    const serverOptions = { onError: errorMod.mapErrorToResponse };
    const allRoutes = [
      ...emergencyServerMod.createEmergencyServiceRoutes(emergencyHandlerMod.emergencyHandler, serverOptions),
    ];
...
const RSS_PROXY_ALLOWED_DOMAINS = new Set([
  ...
  // Emergency variant — Hawaii local news and government feeds
  'api.weather.gov', 'dod.hawaii.gov', 'www.civilbeat.org', 'mauinow.com',
  'bigislandnow.com', 'www.weather.gov', 'www.hawaiinewsnow.com',
  'alerts.weather.gov', 'forecast.weather.gov',
]);
