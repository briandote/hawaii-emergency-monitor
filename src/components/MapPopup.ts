...
export class MapPopup {
  ...
  private renderContent(data: PopupData): string {
    switch (data.type) {
      ...
      case 'shelter':
        return this.renderShelterPopup(data.data as unknown as Shelter);
      case 'roadClosure':
        return this.renderRoadClosurePopup(data.data as unknown as RoadClosure);
      case 'streamGauge':
        return this.renderStreamGaugePopup(data.data as unknown as StreamGauge);
      case 'powerOutage':
        return this.renderPowerOutagePopup(data.data as unknown as PowerOutage);
      case 'evacuationZone':
        return this.renderEvacuationZonePopup(data.data as unknown as { name: string; status: string });
      case 'floodZone':
        return this.renderFloodZonePopup(data.data as unknown as { event: string; severity: string; headline: string; areaDesc: string });
      default:
        return '';
    }
  }
  ...
  // --- Emergency variant popup renderers ---

  private renderShelterPopup(shelter: Shelter): string {
    const statusColor = shelter.status === 'open' ? 'low' : shelter.status === 'limited' ? 'medium' : 'high';
    const features = [
      shelter.petsAllowed ? 'Pets OK' : '',
      shelter.adaAccessible ? 'ADA' : '',
    ].filter(Boolean).join(' · ');
    return `
      <div class="popup-header">
        <span class="popup-title">🏠 ${escapeHtml((shelter.name || 'Shelter').toUpperCase())}</span>
        <span class="popup-badge ${statusColor}">${escapeHtml((shelter.status || 'unknown').toUpperCase())}</span>
        <button class="popup-close" aria-label="Close">×</button>
      </div>
      <div class="popup-body">
        <div class="popup-subtitle">${escapeHtml(shelter.address || '')}</div>
        <div class="popup-stats">
          <div class="popup-stat">
            <span class="stat-label">Type</span>
            <span class="stat-value">${escapeHtml(shelter.type || 'General')}</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">Capacity</span>
            <span class="stat-value">${shelter.capacity || 'N/A'}</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">Island</span>
            <span class="stat-value">${escapeHtml(shelter.island || '')}</span>
          </div>
        </div>
        ${features ? `<p class="popup-description">${escapeHtml(features)}</p>` : ''}
      </div>
    `;
  }

  private renderRoadClosurePopup(closure: RoadClosure): string {
    const statusColor = closure.status === 'closed' ? 'high' : 'medium';
    const updated = closure.updatedAt ? new Date(closure.updatedAt).toLocaleString() : '';
    return `
      <div class="popup-header">
        <span class="popup-title">🚧 ${escapeHtml((closure.road || 'Road Closure').toUpperCase())}</span>
        <span class="popup-badge ${statusColor}">${escapeHtml((closure.status || 'closed').toUpperCase())}</span>
        <button class="popup-close" aria-label="Close">×</button>
      </div>
      <div class="popup-body">
        <div class="popup-subtitle">${escapeHtml(closure.location || '')}</div>
        <div class="popup-stats">
          <div class="popup-stat">
            <span class="stat-label">Reason</span>
            <span class="stat-value">${escapeHtml(closure.reason || 'Unknown')}</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">Detour</span>
            <span class="stat-value">${escapeHtml(closure.detour || 'N/A')}</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">Island</span>
            <span class="stat-value">${escapeHtml(closure.island || '')}</span>
          </div>
        </div>
        ${updated ? `<p class="popup-description">Updated: ${escapeHtml(updated)}</p>` : ''}
      </div>
    `;
  }

  private renderStreamGaugePopup(gauge: StreamGauge): string {
    const ratio = gauge.floodStage > 0 ? gauge.gaugeHeight / gauge.floodStage : 0;
    const statusClass = ratio >= 1.5 ? 'high' : ratio >= 1.0 ? 'elevated' : ratio >= 0.8 ? 'medium' : 'low';
    const statusLabel = ratio >= 1.5 ? 'MAJOR FLOOD' : ratio >= 1.0 ? 'FLOOD' : ratio >= 0.8 ? 'ACTION' : 'NORMAL';
    return `
      <div class="popup-header">
        <span class="popup-title">🌊 ${escapeHtml((gauge.siteName || gauge.siteCode || 'Stream Gauge').toUpperCase())}</span>
        <span class="popup-badge ${statusClass}">${statusLabel}</span>
        <button class="popup-close" aria-label="Close">×</button>
      </div>
      <div class="popup-body">
        <div class="popup-stats">
          <div class="popup-stat">
            <span class="stat-label">Gauge Height</span>
            <span class="stat-value">${Number(gauge.gaugeHeight).toFixed(2)} ft</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">Flood Stage</span>
            <span class="stat-value">${gauge.floodStage > 0 ? Number(gauge.floodStage).toFixed(2) + ' ft' : 'N/A'}</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">Discharge</span>
            <span class="stat-value">${gauge.discharge > 0 ? Number(gauge.discharge).toFixed(0) + ' cfs' : 'N/A'}</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">Site Code</span>
            <span class="stat-value">${escapeHtml(gauge.siteCode || '')}</span>
          </div>
        </div>
        ${gauge.dateTime ? `<p class="popup-description">Observed: ${escapeHtml(new Date(gauge.dateTime).toLocaleString())}</p>` : ''}
      </div>
    `;
  }

  private renderPowerOutagePopup(outage: PowerOutage): string {
    const customers = outage.customersAffected || 0;
    const severityClass = customers > 1000 ? 'high' : customers > 100 ? 'elevated' : 'medium';
    const restoration = outage.estimatedRestoration
      ? new Date(outage.estimatedRestoration).toLocaleString()
      : 'Unknown';
    return `
      <div class="popup-header">
        <span class="popup-title">⚡ ${escapeHtml((outage.area || outage.island || 'Power Outage').toUpperCase())}</span>
        <span class="popup-badge ${severityClass}">${customers.toLocaleString()} AFFECTED</span>
        <button class="popup-close" aria-label="Close">×</button>
      </div>
      <div class="popup-body">
        <div class="popup-stats">
          <div class="popup-stat">
            ...
            <span class="stat-value">${customers.toLocaleString()}</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">Cause</span>
            <span class="stat-value">${escapeHtml(outage.cause || 'Unknown')}</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">Est. Restoration</span>
            <span class="stat-value">${escapeHtml(restoration)}</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">Island</span>
            <span class="stat-value">${escapeHtml(outage.island || '')}</span>
          </div>
        </div>
        ${outage.startTime ? `<p class="popup-description">Started: ${escapeHtml(new Date(outage.startTime).toLocaleString())}</p>` : ''}
      </div>
    `;
  }

  private renderEvacuationZonePopup(zone: { name: string; status: string }): string {
    return `
      <div class="popup-header">
        <span class="popup-title">🚨 ${escapeHtml((zone.name || 'Evacuation Zone').toUpperCase())}</span>
        <span class="popup-badge elevated">${escapeHtml((zone.status || 'ACTIVE').toUpperCase())}</span>
        <button class="popup-close" aria-label="Close">×</button>
      </div>
      <div class="popup-body">
        <p class="popup-description">This area is designated as an evacuation zone.</p>
      </div>
    `;
  }

  private renderFloodZonePopup(zone: { event: string; severity: string; headline: string; areaDesc: string }): string {
    const severityClass = zone.severity === 'Extreme' ? 'high' : zone.severity === 'Severe' ? 'elevated' : 'medium';
    return `
      <div class="popup-header">
        <span class="popup-title">💧 ${escapeHtml((zone.event || 'Flood Zone').toUpperCase())}</span>
        <span class="popup-badge ${severityClass}">${escapeHtml((zone.severity || '').toUpperCase())}</span>
        <button class="popup-close" aria-label="Close">×</button>
      </div>
      <div class="popup-body">
        ${zone.headline ? `<div class="popup-subtitle">${escapeHtml(zone.headline)}</div>` : ''}
        ${zone.areaDesc ? `<p class="popup-description">${escapeHtml(zone.areaDesc)}</p>` : ''}
      </div>
    `;
  }
