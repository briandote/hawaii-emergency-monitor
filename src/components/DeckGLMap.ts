...
import type { Shelter, RoadClosure, StreamGauge, PowerOutage } from '@/generated/server/worldmonitor/emergency/v1/service_server';
...
export class DeckGLMap {
  ...
  // Emergency variant data
  private emergencyShelters: Shelter[] = [];
  private emergencyRoadClosures: RoadClosure[] = [];
  private emergencyStreamGauges: StreamGauge[] = [];
  private emergencyPowerOutages: PowerOutage[] = [];
  private emergencyEvacuationZones: Array<{ polygon: number[][]; name: string; status: string }> = [];
  private emergencyFloodZones: Array<{ polygon: number[][]; event: string; severity: string; headline: string; areaDesc: string }> = [];
  ...
  constructor(container: HTMLElement, initialState: DeckMapState) {
    ...
    // Kick off emergency data fetch for emergency variant
  ...
  private async loadEmergencyData(): Promise<void> {
    ...
    const [sheltersRes, closuresRes, gaugesRes, outagesRes, alertsRes] = await Promise.all([
      fetchJson<{ shelters: Shelter[] }>(`${base}/list-shelters?island=`),
      fetchJson<{ closures: RoadClosure[] }>(`${base}/list-road-closures?island=`),
      fetchJson<{ gauges: StreamGauge[] }>(`${base}/list-stream-gauges?island=`),
      fetchJson<{ outages: PowerOutage[] }>(`${base}/list-power-outages?island=`),
      fetchJson<{ alerts: Array<{ event: string; severity: string; headline: string; description: string; areaDesc: string; polygon: string }> }>(`${base}/list-active-alerts?island=`),
    ]);
    if (sheltersRes?.shelters) { this.emergencyShelters = sheltersRes.shelters; this.setLayerReady('shelters', sheltersRes.shelters.length > 0); }
    if (closuresRes?.closures) { this.emergencyRoadClosures = closuresRes.closures; this.setLayerReady('roadClosures', closuresRes.closures.length > 0); }
    if (gaugesRes?.gauges) { this.emergencyStreamGauges = gaugesRes.gauges; this.setLayerReady('streamGauges', gaugesRes.gauges.length > 0); }
    if (outagesRes?.outages) { this.emergencyPowerOutages = outagesRes.outages; this.setLayerReady('powerOutages', outagesRes.outages.length > 0); }
    // Parse NWS alert polygons into flood zones
    if (alertsRes?.alerts) {
      const floodKeywords = ['flood', 'flash flood', 'coastal flood', 'river flood', 'tsunami'];
      const floodAlerts = alertsRes.alerts.filter(a =>
        floodKeywords.some(kw => (a.event || '').toLowerCase().includes(kw))
      );
      const zones = floodAlerts
        .filter(a => a.polygon)
        .map(a => {
          const ring = a.polygon.split(' ').map(pair => {
            const [lat, lon] = pair.split(',').map(Number);
            return [lon, lat] as number[];
          }).filter(c => Number.isFinite(c[0]) && Number.isFinite(c[1]));
          return { polygon: ring, event: a.event, severity: a.severity, headline: a.headline, areaDesc: a.areaDesc };
        })
        .filter(z => z.polygon.length >= 3);
      this.emergencyFloodZones = zones;
      this.setLayerReady('floodZones', zones.length > 0);
    }
    this.render();
  }
  ...
  private buildLayers(): LayersList {
    ...
    // Emergency variant layers
    if (mapLayers.floodZones && this.emergencyFloodZones.length > 0) {
      layers.push(this.createFloodZonesLayer());
    }
    if (mapLayers.evacuationZones && this.emergencyEvacuationZones.length > 0) {
      layers.push(this.createEvacuationZonesLayer());
    }
    if (mapLayers.shelters && this.emergencyShelters.length > 0) {
      layers.push(this.createSheltersLayer());
    }
    if (mapLayers.roadClosures && this.emergencyRoadClosures.length > 0) {
      layers.push(this.createRoadClosuresLayer());
    }
    if (mapLayers.streamGauges && this.emergencyStreamGauges.length > 0) {
      layers.push(this.createStreamGaugesLayer());
    }
    if (mapLayers.powerOutages && this.emergencyPowerOutages.length > 0) {
      layers.push(this.createPowerOutagesLayer());
    }
  ...
  // --- Emergency variant layer creation methods ---

  private createSheltersLayer(): ScatterplotLayer<Shelter> {
    return new ScatterplotLayer<Shelter>({
      id: 'shelters-layer',
      data: this.emergencyShelters,
      getPosition: (d) => [d.longitude, d.latitude],
      getRadius: 500,
      getFillColor: (d) => {
        switch (d.status) {
          case 'open': return [34, 197, 94, 220] as [number, number, number, number];
          case 'limited': return [234, 179, 8, 220] as [number, number, number, number];
          case 'full': case 'closed': return [239, 68, 68, 220] as [number, number, number, number];
          default: return [107, 114, 128, 200] as [number, number, number, number]; // standby/unknown
        }
      },
  ...
  private createRoadClosuresLayer(): ScatterplotLayer<RoadClosure> {
    return new ScatterplotLayer<RoadClosure>({
      id: 'road-closures-layer',
      data: this.emergencyRoadClosures,
      getPosition: (d) => [d.longitude, d.latitude],
      getRadius: 400,
      getFillColor: (d) => {
        if (d.status === 'closed') return [239, 68, 68, 220] as [number, number, number, number];
        return [234, 179, 8, 220] as [number, number, number, number]; // restricted
      },
      getLineColor: [255, 255, 255, 180],
      stroked: true,
      lineWidthMinPixels: 2,
  ...
  private createStreamGaugesLayer(): ScatterplotLayer<StreamGauge> {
    return new ScatterplotLayer<StreamGauge>({
      id: 'stream-gauges-layer',
      data: this.emergencyStreamGauges,
      getPosition: (d) => [d.longitude, d.latitude],
      getRadius: (d) => {
        if (d.floodStage <= 0) return 400;
        const ratio = d.gaugeHeight / d.floodStage;
        return 300 + Math.min(ratio, 3) * 300;
      },
      getFillColor: (d) => {
        if (d.floodStage <= 0) return [107, 114, 128, 200] as [number, number, number, number];
        const ratio = d.gaugeHeight / d.floodStage;
        if (ratio >= 1.5) return [239, 68, 68, 220] as [number, number, number, number]; // major flood
        if (ratio >= 1.0) return [249, 115, 22, 220] as [number, number, number, number]; // flood
        if (ratio >= 0.8) return [234, 179, 8, 220] as [number, number, number, number]; // action
        return [34, 197, 94, 200] as [number, number, number, number]; // normal
      },
  ...
  private createPowerOutagesLayer(): ScatterplotLayer {
    // Power outages don't have lat/lng in proto; use island centroid mapping
    const islandCentroids: Record<string, [number, number]> = {
      oahu: [-157.86, 21.44],
      maui: [-156.33, 20.80],
      hawaii: [-155.52, 19.60],
      kauai: [-159.37, 22.07],
      molokai: [-157.00, 21.13],
      lanai: [-156.92, 20.83],
    };
    type OutageWithCoord = PowerOutage & { _lon: number; _lat: number };
    const data: OutageWithCoord[] = this.emergencyPowerOutages
      .map(o => {
        const center = islandCentroids[o.island?.toLowerCase()] ?? islandCentroids.oahu!;
        // Jitter slightly so co-located outages don't stack
        const jitter = () => (Math.random() - 0.5) * 0.05;
        return { ...o, _lon: center![0] + jitter(), _lat: center![1] + jitter() };
      });
    return new ScatterplotLayer<OutageWithCoord>({
      id: 'power-outages-layer',
      data,
      getPosition: (d) => [d._lon, d._lat],
      getRadius: (d) => {
        const customers = d.customersAffected || 0;
        return 300 + Math.min(customers / 50, 1500);
      },
      getFillColor: (d) => {
        const c = d.customersAffected || 0;
        if (c > 1000) return [239, 68, 68, 220] as [number, number, number, number];
        if (c > 100) return [249, 115, 22, 220] as [number, number, number, number];
  ...
  private createEvacuationZonesLayer(): PolygonLayer {
    return new PolygonLayer({
      id: 'evacuation-zones-layer',
      data: this.emergencyEvacuationZones,
      getPolygon: (d: { polygon: number[][] }) => d.polygon,
      getFillColor: [251, 146, 60, 77], // rgba(251, 146, 60, 0.3)
  ...
  private createFloodZonesLayer(): PolygonLayer {
    return new PolygonLayer({
      id: 'flood-zones-layer',
      data: this.emergencyFloodZones,
      getPolygon: (d: { polygon: number[][] }) => d.polygon,
      getFillColor: [59, 130, 246, 64], // rgba(59, 130, 246, 0.25)
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    switch (layerId) {
      ...
      case 'shelters-layer': {
        const statusColor = obj.status === 'open' ? '#22c55e' : obj.status === 'limited' ? '#eab308' : obj.status === 'full' || obj.status === 'closed' ? '#ef4444' : '#6b7280';
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/><span style="color:${statusColor}">${text((obj.status || 'unknown').toUpperCase())}</span>${obj.capacity ? ` · Cap: ${obj.capacity}` : ''}</div>` };
      }
      case 'road-closures-layer': {
        const rcColor = obj.status === 'closed' ? '#ef4444' : '#eab308';
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.road)}</strong><br/><span style="color:${rcColor}">${text((obj.status || 'closed').toUpperCase())}</span><br/>${text(obj.reason || '')}</div>` };
      }
      case 'stream-gauges-layer': {
        const sgRatio = obj.floodStage > 0 ? (obj.gaugeHeight / obj.floodStage) : 0;
        const sgColor = sgRatio >= 1.5 ? '#ef4444' : sgRatio >= 1.0 ? '#f97316' : sgRatio >= 0.8 ? '#eab308' : '#22c55e';
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.siteName)}</strong><br/>Height: <span style="color:${sgColor}">${Number(obj.gaugeHeight).toFixed(1)} ft</span>${obj.floodStage > 0 ? ` / Flood: ${Number(obj.floodStage).toFixed(1)} ft` : ''}</div>` };
      }
      case 'power-outages-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.area || obj.island || 'Power Outage')}</strong><br/>${obj.customersAffected ? `${obj.customersAffected.toLocaleString()} customers` : ''}${obj.cause ? `<br/>${text(obj.cause)}` : ''}</div>` };
      case 'evacuation-zones-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.name || 'Evacuation Zone')}</strong><br/>${text(obj.status || 'Active')}</div>` };
      case 'flood-zones-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.event || 'Flood Zone')}</strong><br/>${text(obj.severity || '')}<br/>${text((obj.areaDesc || '').slice(0, 80))}${(obj.areaDesc || '').length > 80 ? '...' : ''}</div>` };
      default:
        return null;
    }
  }
  ...
  private handleClick(info: PickingInfo): void {
    ...
    const layerToPopupType: Record<string, PopupType> = {
      ...
      'shelters-layer': 'shelter',
      'road-closures-layer': 'roadClosure',
      'stream-gauges-layer': 'streamGauge',
      'power-outages-layer': 'powerOutage',
      'evacuation-zones-layer': 'evacuationZone',
      'flood-zones-layer': 'floodZone',
    };
      ...
      : SITE_VARIANT === 'finance'
        ...
        : SITE_VARIANT === 'happy'
          ...
          : SITE_VARIANT === 'commodity'
            ...
            : isEmergencyVariant
              ? [
                { shape: shapes.circle('rgb(34, 197, 94)'), label: 'Shelter (Open)', layerKey: 'shelters' as keyof MapLayers },
                { shape: shapes.circle('rgb(239, 68, 68)'), label: 'Road Closure', layerKey: 'roadClosures' as keyof MapLayers },
                { shape: shapes.circle('rgb(59, 130, 246)'), label: 'Stream Gauge', layerKey: 'streamGauges' as keyof MapLayers },
                { shape: shapes.circle('rgb(249, 115, 22)'), label: 'Power Outage', layerKey: 'powerOutages' as keyof MapLayers },
                { shape: shapes.square('rgb(251, 146, 60)'), label: 'Evacuation Zone', layerKey: 'evacuationZones' as keyof MapLayers },
                { shape: shapes.square('rgb(59, 130, 246)'), label: 'Flood Zone', layerKey: 'floodZones' as keyof MapLayers },
              ]
              : [
  ...
  public setShelters(shelters: Shelter[]): void {
    this.emergencyShelters = shelters;
    this.render();
  }

  public setRoadClosures(closures: RoadClosure[]): void {
    this.emergencyRoadClosures = closures;
    this.render();
  }

  public setStreamGauges(gauges: StreamGauge[]): void {
    this.emergencyStreamGauges = gauges;
    this.render();
  }

  public setPowerOutages(outages: PowerOutage[]): void {
    this.emergencyPowerOutages = outages;
    this.render();
  }

  public setEvacuationZones(zones: Array<{ polygon: number[][]; name: string; status: string }>): void {
    this.emergencyEvacuationZones = zones;
    this.render();
  }

  public setFloodZones(zones: Array<{ polygon: number[][]; event: string; severity: string; headline: string; areaDesc: string }>): void {
    this.emergencyFloodZones = zones;
    this.render();
  }
