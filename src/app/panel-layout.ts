...
export class PanelLayoutManager implements AppModule {
  ...
  constructor(ctx: AppContext, callbacks: PanelLayoutManagerCallbacks) {
    this.ctx = ctx;
    void callbacks; // stored for future use
  ...
  destroy(): void {
    ...
    // Clean up emergency variant panels
    this.ctx.activeAlertsPanel?.destroy();
    this.ctx.activeAlertsPanel = null;
    this.ctx.shelterMapPanel?.destroy();
    this.ctx.shelterMapPanel = null;
    this.ctx.streamGaugePanel?.destroy();
    this.ctx.streamGaugePanel = null;
    this.ctx.roadClosurePanel?.destroy();
    this.ctx.roadClosurePanel = null;
    this.ctx.powerOutagePanel?.destroy();
    this.ctx.powerOutagePanel = null;
  ...
  /** No-op for emergency variant — live news panel not used. */
  mountLiveNewsIfReady(): void {
    // Not applicable for emergency variant
  }
  ...
  private createPanels(): void {
    ...
    void loadFromStorage<string>(STORAGE_KEYS.mapMode, 'flat'); // preferGlobe - reserved for globe mode
    this.ctx.map = new MapContainer(mapContainer, {
      zoom: this.ctx.isMobile ? 2.5 : 8,
      pan: { x: 0, y: 0 },
    ...
    // Emergency variant panels (lazy-loaded)
    this.lazyPanel('active-alerts', () =>
      import('@/components/ActiveAlertsPanel').then(m => {
        const p = new m.ActiveAlertsPanel();
        void p.fetchData();
        p.startAutoRefresh();
        this.ctx.activeAlertsPanel = p;
        return p;
      }),
    );

    this.lazyPanel('shelter-map', () =>
      import('@/components/ShelterMapPanel').then(m => {
        const p = new m.ShelterMapPanel();
        void p.fetchData();
        p.startAutoRefresh();
        this.ctx.shelterMapPanel = p;
        return p;
      }),
    );

    this.lazyPanel('stream-gauges', () =>
      import('@/components/StreamGaugePanel').then(m => {
        const p = new m.StreamGaugePanel();
        void p.fetchData();
        p.startAutoRefresh();
        this.ctx.streamGaugePanel = p;
        return p;
      }),
    );

    this.lazyPanel('road-closures', () =>
      import('@/components/RoadClosurePanel').then(m => {
        const p = new m.RoadClosurePanel();
        void p.fetchData();
        p.startAutoRefresh();
        this.ctx.roadClosurePanel = p;
        return p;
      }),
    );

    this.lazyPanel('power-outages', () =>
      import('@/components/PowerOutagePanel').then(m => {
        const p = new m.PowerOutagePanel();
        void p.fetchData();
        p.startAutoRefresh();
        this.ctx.powerOutagePanel = p;
        return p;
      }),
    );

    this.lazyPanel('local-news', () =>
      import('@/components/HawaiiNewsPanel').then(m => {
        const p = new m.HawaiiNewsPanel();
        return p;
      }),
    );

    this.lazyPanel('weather-radar', () =>
      import('@/components/WeatherRadarPanel').then(m => {
        const p = new m.WeatherRadarPanel();
        void p.fetchData();
        return p;
      }),
    );

    this.lazyPanel('emergency-resources', () =>
      import('@/components/EmergencyResourcesPanel').then(m => {
        const p = new m.EmergencyResourcesPanel();
        return p;
      }),
    );

    this.lazyPanel('event-timeline', () =>
      import('@/components/EventTimelinePanel').then(m => {
        const p = new m.EventTimelinePanel();
        return p;
      }),
    );

    this.lazyPanel('community-reports', () =>
      import('@/components/CommunityReportsPanel').then(m => {
        const p = new m.CommunityReportsPanel();
        return p;
      }),
    );
  ...
  private applyTimeRangeFilterToNewsPanels(): void {
    // No news panels in emergency variant — no-op
  }
  ...
  private applyInitialUrlState(): void {
    ...
    if (view && SITE_VARIANT !== 'emergency') {
  ...
  // Related asset handlers removed — not applicable for emergency variant
