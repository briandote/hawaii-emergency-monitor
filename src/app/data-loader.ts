...
// Emergency variant: most panel types removed — using `any` casts in method bodies
// that reference panels which may or may not exist at runtime.
...
// GeoHubsPanel, TechHubsPanel removed — emergency variant
...
export class DataLoaderManager implements AppModule {
  ...
  private shouldShowIntelligenceNotifications(): boolean {
    return false; // No intelligence notifications in emergency variant
  }
    ...
    if (SITE_VARIANT === 'full') {
      try {
        ...
        if (cached && cached.cii.length > 0) {
          (this.ctx.panels['cii'] as any)?.renderFromCached(cached);
  ...
  renderNewsForCategory(category: string, items: NewsItem[]): void {
    ...
    const panel = (this.ctx.newsPanels as any)[category];
    if (!panel) return;
  ...
  private async loadNewsCategory(category: string, feeds: typeof FEEDS.politics, digest?: ListFeedDigestResponse | null): Promise<NewsItem[]> {
    try {
      ...
      if (enabledFeeds.length === 0) {
        ...
        ((this.ctx.statusPanel) as any)?.updateFeed(category.charAt(0).toUpperCase() + category.slice(1), {
          status: 'ok',
          itemCount: 0,
        });
        return [];
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('RSS2JSON', { status: 'ok' });
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('RSS2JSON', { status: 'error' });
      delete this.ctx.newsByCategory[category];
      return [];
    }
  }
  ...
  async loadNews(): Promise<void> {
    ...
    if (SITE_VARIANT === 'full') {
      ...
      const intelPanel = (this.ctx.newsPanels as any)['intel'];
      if (enabledIntelSources.length === 0) {
        delete this.ctx.newsByCategory['intel'];
        if (intelPanel) intelPanel.showError(t('common.allIntelSourcesDisabled'));
        ((this.ctx.statusPanel) as any)?.updateFeed('Intel', { status: 'ok', itemCount: 0 });
        ...
        ((this.ctx.statusPanel) as any)?.updateFeed('Intel', { status: 'ok', itemCount: intel.length });
        ...
        if (staleIntel.length > 0) {
          ...
          ((this.ctx.statusPanel) as any)?.updateFeed('Intel', { status: 'ok', itemCount: staleIntel.length });
          ...
          delete this.ctx.newsByCategory['intel'];
          ((this.ctx.statusPanel) as any)?.updateFeed('Intel', { status: 'error', errorMessage: 'Digest unavailable' });
        } else {
    ...
    try {
      ...
      const insightsPanel = this.ctx.panels['insights'] as any | undefined;
      insightsPanel?.updateInsights(this.ctx.latestClusters);

      (this.ctx.panels['geo-hubs'] as any)
        ?.setActivities(getTopActiveGeoHubs(this.ctx.latestClusters));
      (this.ctx.panels['tech-hubs'] as any)
  ...
  async loadMarkets(): Promise<void> {
    try {
      ...
      if (stocksResult.rateLimited && stocksResult.data.length === 0) {
        ...
        ((this.ctx.statusPanel) as any)?.updateApi('Finnhub', { status: 'error' });
        if (stocksResult.data.length === 0) {
          this.ctx.panels['markets']?.showConfigError(finnhubConfigMsg);
        }
      } else {
        ((this.ctx.statusPanel) as any)?.updateApi('Finnhub', { status: 'ok' });
      }
    ...
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('CoinGecko', { status: crypto.length > 0 ? 'ok' : 'error' });
    } catch {
      ((this.ctx.statusPanel) as any)?.updateApi('CoinGecko', { status: 'error' });
    }
  ...
  async loadPredictions(): Promise<void> {
    try {
      ...
      (this.ctx.panels['polymarket'] as any | undefined)?.renderPredictions(predictions);

      ((this.ctx.statusPanel) as any)?.updateFeed('Polymarket', { status: 'ok', itemCount: predictions.length });
      ((this.ctx.statusPanel) as any)?.updateApi('Polymarket', { status: 'ok' });
      ...
      ((this.ctx.statusPanel) as any)?.updateFeed('Polymarket', { status: 'error', errorMessage: String(error) });
      ((this.ctx.statusPanel) as any)?.updateApi('Polymarket', { status: 'error' });
  ...
  async loadNatural(): Promise<void> {
    ...
    if (earthquakeResult.status === 'fulfilled') {
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('USGS', { status: 'ok' });
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('USGS', { status: 'error' });
      dataFreshness.recordError('usgs', String(earthquakeResult.reason));
    }

    if (eonetResult.status === 'fulfilled') {
      this.ctx.map?.setNaturalEvents(eonetResult.value);
      ((this.ctx.statusPanel) as any)?.updateFeed('EONET', {
        status: 'ok',
        itemCount: eonetResult.value.length,
      });
      ((this.ctx.statusPanel) as any)?.updateApi('NASA EONET', { status: 'ok' });
    } else {
      this.ctx.map?.setNaturalEvents([]);
      ((this.ctx.statusPanel) as any)?.updateFeed('EONET', { status: 'error', errorMessage: String(eonetResult.reason) });
      ((this.ctx.statusPanel) as any)?.updateApi('NASA EONET', { status: 'error' });
    }
  ...
  async loadTechEvents(): Promise<void> {
    ...
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateFeed('Tech Events', { status: 'ok', itemCount: mapEvents.length });
      ...
      ((this.ctx.statusPanel) as any)?.updateFeed('Tech Events', { status: 'error', errorMessage: String(error) });
    }
  }
  ...
  async loadWeatherAlerts(): Promise<void> {
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateFeed('Weather', { status: 'ok', itemCount: alerts.length });
      dataFreshness.recordUpdate('weather', alerts.length);
    } catch (error) {
      this.ctx.map?.setLayerReady('weather', false);
      ((this.ctx.statusPanel) as any)?.updateFeed('Weather', { status: 'error' });
    ...
    tasks.push((async () => {
      try {
        ...
        if (this.ctx.mapLayers.outages) {
          ...
          ((this.ctx.statusPanel) as any)?.updateFeed('NetBlocks', { status: 'ok', itemCount: outages.length });
        }
        (this.ctx.panels['internet-disruptions'] as any)?.setOutages(outages);
        fetchTrafficAnomalies().then(r => {
          this.ctx.map?.setTrafficAnomalies(r.anomalies);
          (this.ctx.panels['internet-disruptions'] as any)?.setAnomalies(r.anomalies);
        }).catch(() => {});
        fetchDdosAttacks().then(r => {
          this.ctx.map?.setDdosLocations(r.topTargetLocations ?? []);
          (this.ctx.panels['internet-disruptions'] as any)?.setDdos(r);
        }).catch(() => {});
      } catch (error) {
    ...
    const protestsTask = (async (): Promise<SocialUnrestEvent[]> => {
      try {
        ...
        if (this.ctx.mapLayers.protests) {
          ...
          ((this.ctx.statusPanel) as any)?.updateFeed('Protests', {
            status: 'ok',
    ...
    tasks.push((async () => {
      try {
        ...
        if (this.ctx.mapLayers.military) {
          ...
          ((this.ctx.statusPanel) as any)?.updateFeed('Military', {
    ...
    if (!isDesktopRuntime()) {
      tasks.push((async () => {
        try {
          ...
          ((this.ctx.statusPanel) as any)?.updateFeed('GPS Jam', { status: 'ok', itemCount: data.hexes.length });
          dataFreshness.recordUpdate('gpsjam', data.hexes.length);
        } catch (error) {
          this.ctx.map?.setLayerReady('gpsJamming', false);
          ((this.ctx.statusPanel) as any)?.updateFeed('GPS Jam', { status: 'error' });
  ...
  async loadOutages(): Promise<void> {
    ...
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateFeed('NetBlocks', { status: 'error' });
      dataFreshness.recordError('outages', String(error));
    }
  }
  ...
  async loadCyberThreats(): Promise<void> {
    ...
    if (this.ctx.cyberThreatsCache) {
      ...
      this.refreshCiiAndBrief();
      ((this.ctx.statusPanel) as any)?.updateFeed('Cyber Threats', { status: 'ok', itemCount: this.ctx.cyberThreatsCache.length });
      return;
    }
    ...
    try {
      ...
      this.refreshCiiAndBrief();
      ((this.ctx.statusPanel) as any)?.updateFeed('Cyber Threats', { status: 'ok', itemCount: threats.length });
      ((this.ctx.statusPanel) as any)?.updateApi('Cyber Threats API', { status: 'ok' });
      dataFreshness.recordUpdate('cyber_threats', threats.length);
    } catch (error) {
      this.ctx.map?.setLayerReady('cyberThreats', false);
      ((this.ctx.statusPanel) as any)?.updateFeed('Cyber Threats', { status: 'error', errorMessage: String(error) });
      ((this.ctx.statusPanel) as any)?.updateApi('Cyber Threats API', { status: 'error' });
  ...
  async loadAisSignals(): Promise<void> {
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateFeed('Shipping', { status: 'error', errorMessage: String(error) });
      ((this.ctx.statusPanel) as any)?.updateApi('AISStream', { status: 'error' });
  ...
  async loadCableActivity(): Promise<void> {
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateFeed('CableOps', { status: 'ok', itemCount });
    } catch {
      ((this.ctx.statusPanel) as any)?.updateFeed('CableOps', { status: 'error' });
    }
  }
  ...
  async loadCableHealth(): Promise<void> {
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateFeed('CableHealth', { status: 'ok', itemCount: faultCount + degradedCount });
    } catch {
      ((this.ctx.statusPanel) as any)?.updateFeed('CableHealth', { status: 'error' });
    }
  }
  ...
  async loadProtests(): Promise<void> {
    if (this.ctx.intelligenceCache.protests) {
      ...
      if (status.acledConfigured === true) {
        ((this.ctx.statusPanel) as any)?.updateApi('ACLED', { status: 'ok' });
      } else if (status.acledConfigured === null) {
        ((this.ctx.statusPanel) as any)?.updateApi('ACLED', { status: 'warning' });
      }
      ((this.ctx.statusPanel) as any)?.updateApi('GDELT Doc', { status: 'ok' });
      if (protestData.sources.gdelt > 0) dataFreshness.recordUpdate('gdelt_doc', protestData.sources.gdelt);
      return;
    }
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateFeed('Protests', { status: 'error', errorMessage: String(error) });
      ((this.ctx.statusPanel) as any)?.updateApi('ACLED', { status: 'error' });
      ((this.ctx.statusPanel) as any)?.updateApi('GDELT Doc', { status: 'error' });
  ...
  async loadFlightDelays(): Promise<void> {
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateFeed('Flights', {
        status: 'ok',
        itemCount: delays.length,
      });
      ((this.ctx.statusPanel) as any)?.updateApi('FAA', { status: 'ok' });
    } catch (error) {
      this.ctx.map?.setLayerReady('flights', false);
      ((this.ctx.statusPanel) as any)?.updateFeed('Flights', { status: 'error', errorMessage: String(error) });
      ((this.ctx.statusPanel) as any)?.updateApi('FAA', { status: 'error' });
    }
  }
  ...
  async loadMilitary(): Promise<void> {
    if (this.ctx.intelligenceCache.military) {
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('OpenSky', { status: 'ok' });
      return;
    }
    try {
      if (isMilitaryVesselTrackingConfigured()) {
      ...
      ((this.ctx.statusPanel) as any)?.updateFeed('Military', { status: 'error', errorMessage: String(error) });
      ((this.ctx.statusPanel) as any)?.updateApi('OpenSky', { status: 'error' });
      dataFreshness.recordError('opensky', String(error));
    }
  }
  ...
  private async loadCachedPosturesForBanner(): Promise<void> {
    try {
      ...
      if (data && data.postures.length > 0) {
        ...
        const posturePanel = this.ctx.panels['strategic-posture'] as any | undefined;
  ...
  async loadFredData(): Promise<void> {
    ...
    if (cbInfo.onCooldown) {
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('FRED', { status: 'error' });
      return;
    }
    ...
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('FRED', { status: 'ok' });
  ...
  async loadOilAnalytics(): Promise<void> {
    ...
    try {
      ...
      if (data.status === 'fulfilled') {
        ...
        ((this.ctx.statusPanel) as any)?.updateApi('EIA', { status: hasData ? 'ok' : 'error' });
        if (hasData) {
        ...
        ((this.ctx.statusPanel) as any)?.updateApi('EIA', { status: 'error' });
  ...
  async loadGovernmentSpending(): Promise<void> {
    ...
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('USASpending', { status: data.awards?.length > 0 ? 'ok' : 'error' });
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('USASpending', { status: 'error' });
  ...
  async loadBisData(): Promise<void> {
    ...
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('BIS', { status: hasData ? 'ok' : 'error' });
      if (hasData) {
        dataFreshness.recordUpdate('bis', data.policyRates?.length ?? 0);
      }
    } catch (e) {
      console.error('[App] BIS data failed:', e);
      ((this.ctx.statusPanel) as any)?.updateApi('BIS', { status: 'error' });
  ...
  async loadBlsData(): Promise<void> {
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        ((this.ctx.statusPanel) as any)?.updateApi('BLS-Series', { status: 'ok' });
        dataFreshness.recordUpdate('bls', data.length);
      } else {
        ((this.ctx.statusPanel) as any)?.updateApi('BLS-Series', { status: 'error' });
      }
    } catch (e) {
  ...
  async loadTradePolicy(): Promise<void> {
    ...
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('WTO', { status: anyUnavailable ? 'warning' : wtoItems > 0 ? 'ok' : 'error' });
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('WTO', { status: 'error' });
  ...
  async loadSupplyChain(): Promise<void> {
    ...
    try {
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('SupplyChain', { status: anyUnavailable ? 'warning' : totalItems > 0 ? 'ok' : 'error' });
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('SupplyChain', { status: 'error' });
  ...
  async loadDiseaseOutbreaks(): Promise<void> {
    try {
      ...
      if (data.outbreaks?.length) {
        const panel = this.ctx.panels['disease-outbreaks'] as any | undefined;
  ...
  async loadSocialVelocity(): Promise<void> {
    try {
      ...
      if (data.posts?.length) {
        const panel = this.ctx.panels['social-velocity'] as any | undefined;
  ...
  updateMonitorResults(): void {
    const monitorPanel = this.ctx.panels['monitors'] as any | undefined;
  ...
  async loadFirmsData(): Promise<void> {
    try {
      ...
      if (fireResult.skipped) {
        ...
        ((this.ctx.statusPanel) as any)?.updateApi('FIRMS', { status: 'error' });
        return;
      }
      ...
      ((this.ctx.statusPanel) as any)?.updateApi('FIRMS', { status: 'ok' });
    } catch (e) {
  ...
  async loadPizzInt(): Promise<void> {
    try {
      ...
      if (status.locationsMonitored === 0) {
        ((this.ctx.pizzintIndicator) as any)?.hide();
        ((this.ctx.statusPanel) as any)?.updateApi('PizzINT', { status: 'error' });
        dataFreshness.recordError('pizzint', 'No monitored locations returned');
        return;
      }

      ((this.ctx.pizzintIndicator) as any)?.show();
      ((this.ctx.pizzintIndicator) as any)?.updateStatus(status);
      ((this.ctx.pizzintIndicator) as any)?.updateTensions(tensions);
      ((this.ctx.statusPanel) as any)?.updateApi('PizzINT', { status: 'ok' });
  ...
  async loadSanctionsPressure(): Promise<void> {
    try {
      ...
      if (result.totalCount > 0) {
        ...
        ((this.ctx.statusPanel) as any)?.updateApi('OFAC', { status: result.newEntryCount > 0 ? 'warning' : 'ok' });
      } else {
        ((this.ctx.statusPanel) as any)?.updateApi('OFAC', { status: 'error' });
      }
    } catch (error) {
