...
const FULL_MAP_LAYERS: MapLayers = {
  ...
  // Emergency layers (disabled in full variant)
  shelters: false,
  roadClosures: false,
  streamGauges: false,
  powerOutages: false,
...
const TECH_MAP_LAYERS: MapLayers = {
  ...
  webcams: false,
  weatherRadar: false, diseaseOutbreaks: false,
  // Emergency layers (disabled in tech variant)
  shelters: false,
  roadClosures: false,
  streamGauges: false,
  powerOutages: false,
...
const FINANCE_MAP_LAYERS: MapLayers = {
  ...
  webcams: false,
  weatherRadar: false, diseaseOutbreaks: false,
  // Emergency layers (disabled in finance variant)
  shelters: false,
  roadClosures: false,
  streamGauges: false,
  powerOutages: false,
...
// ============================================
// EMERGENCY VARIANT (Hawaii Disaster Response)
// ============================================
const EMERGENCY_PANELS: Record<string, PanelConfig> = {
  map: { name: 'Hawaii Emergency Map', enabled: true, priority: 1 },
  'active-alerts': { name: 'Active Alerts', enabled: true, priority: 1 },
  'weather-radar': { name: 'Weather Radar', enabled: true, priority: 1 },
  'shelter-map': { name: 'Shelters', enabled: true, priority: 1 },
  'road-closures': { name: 'Road Closures', enabled: true, priority: 1 },
  'power-outages': { name: 'Power Outages', enabled: true, priority: 1 },
  'stream-gauges': { name: 'Stream Gauges', enabled: true, priority: 1 },
  'local-news': { name: 'Hawaii News', enabled: true, priority: 1 },
  'emergency-resources': { name: 'Emergency Resources', enabled: true, priority: 2 },
  'event-timeline': { name: 'Event Timeline', enabled: true, priority: 2 },
  'community-reports': { name: 'Community Reports', enabled: true, priority: 2 },
};

const EMERGENCY_MAP_LAYERS: MapLayers = {
  gpsJamming: false,
  satellites: false,
  ...
  commodityPorts: false,
  webcams: false,
  weatherRadar: true, diseaseOutbreaks: false,
  // Emergency layers (enabled)
  shelters: true,
  roadClosures: true,
  streamGauges: true,
  powerOutages: true,
...
const EMERGENCY_MOBILE_MAP_LAYERS: MapLayers = {
  gpsJamming: false,
  satellites: false,
  ...
  webcams: false,
  weatherRadar: true, diseaseOutbreaks: false,
  // Emergency layers (limited on mobile)
  shelters: true,
  roadClosures: true,
  streamGauges: false,
  powerOutages: true,
...
export const VARIANT_DEFAULTS: Record<string, string[]> = {
  ...
  emergency: Object.keys(EMERGENCY_PANELS),
};
  ...
  : SITE_VARIANT === 'tech'
    ...
    : SITE_VARIANT === 'finance'
      ...
      : SITE_VARIANT === 'commodity'
        ...
        : SITE_VARIANT === 'emergency'
          ? EMERGENCY_MAP_LAYERS
          : FULL_MAP_LAYERS;
...
export const PANEL_CATEGORY_MAP: Record<string, { labelKey: string; panelKeys: string[]; variants?: string[] }> = {
  ...
  // Emergency variant
  emergencyAlerts: {
    labelKey: 'header.panelCatEmergencyAlerts',
    panelKeys: ['active-alerts', 'weather-radar', 'local-news'],
    variants: ['emergency'],
  },
  emergencyInfra: {
    labelKey: 'header.panelCatEmergencyInfra',
    panelKeys: ['shelter-map', 'road-closures', 'power-outages', 'stream-gauges'],
    variants: ['emergency'],
  },
  emergencyCommunity: {
    labelKey: 'header.panelCatEmergencyCommunity',
    panelKeys: ['emergency-resources', 'event-timeline', 'community-reports'],
    variants: ['emergency'],
  },
};
